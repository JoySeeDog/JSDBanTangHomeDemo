...
CGFloat const DAPagesContainerTopBarItemViewWidth = 76.;
CGFloat const DAPagesContainerTopBarItemsOffset = 56.;
...
//        UIView* lineView = [[UIView alloc] initWithFrame:CGRectMake(0.f, 29.5f, self.frame.size.width, 0.5f)];
//        lineView.backgroundColor = UIColorFromRGB(0Xcecece);
...
- (CGFloat)itemCenterXForSelectedItemAtIndex:(NSUInteger)index
{
    UIButton *currentItem = self.itemViews[index];
    return currentItem.center.x;
}
...
{
    CGRect frame = CGRectMake(0., 0., DAPagesContainerTopBarItemViewWidth*SCALE_6, CGRectGetHeight(self.frame));
...
- (void)layoutItemViews
{
    ...
    for (NSUInteger i = 0; i < self.itemViews.count; i++) {
        ...
        if ([self.itemViews count]>4) {
            ...
            x += width + DAPagesContainerTopBarItemsOffset*SCALE_6;
        }
        else
        {
            CGFloat width = SCREEN_WIDTH/[self.itemViews count];
            UIView *itemView = self.itemViews[i];
            itemView.frame = CGRectMake(x, 0., width, CGRectGetHeight(self.frame));
            x += width;
        }
